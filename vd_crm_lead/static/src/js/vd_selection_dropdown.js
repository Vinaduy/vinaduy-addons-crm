...
export class VdSelectionDropdown extends Component {
    ...
    get options() {
        ...
        const all = fieldDef && fieldDef.selection ? fieldDef.selection : [];
        ...
        if (this.props.filterField && this.props.filterMap) {
            ...
            if (Array.isArray(allowed)) {
                return all.filter(([key, _]) => allowed.includes(key));
            }
            // Nếu kind chưa chọn hoặc không có trong map → không cho chọn gì
            return [];
        }
        return all;
    }
